...
occs <- read.csv(paste0('./int_data/', int_sp[task_id], '.csv'))
occs <- occs[,-1]

occs_nat <- read.csv(paste0('./occ_data/', int_sp[task_id], '.csv'))
occs_nat <- occs_nat[,-1]
occs_full <- rbind(occs, occs_nat)

# reading in accessible area
acc_area <- st_read(dsn = './int_acc_area', layer = int_sp[task_id])
acc_area <- st_set_crs(acc_area, target_crs)
acc_area <- as_spatvector(acc_area)

acc_area_nat <- st_read(dsn = './nat_acc_area', layer = int_sp[task_id])
acc_area_nat <- st_set_crs(acc_area_nat, target_crs)
acc_area_nat <- as_spatvector(acc_area_nat)
