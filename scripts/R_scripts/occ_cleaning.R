...
# finish species
# host_list <- read.table('./base_data/final_finish_list.txt')
# host_list <- host_list$x
...
print("beginning array")
  ...
  gbif_data <- occ_data(scientificName = host_list[task_id], hasCoordinate = T,
                         coordinateUncertaintyInMeters = '0,10000', limit = 100)
    ...
    gbif_data$eventDate <- gsub("T.*", '', gbif_data$eventDate)
      ...
      print(paste0("no gbif data for ", host_list[task_id]))
    }
  
    # retrieving idigbio data
    idigbio_data <- idig_search_records(rq = list("scientificname" = host_list[task_id], 
    ...
    idigbio_data$eventDate <- gsub("T.*", '', idigbio_data$eventDate)
      ...
      print(paste0("no idigbio data for ", host_list[task_id]))
    }
    ...
    species <- rep(host_list[task_id], times = nrow(all_data))
    all_data <- cbind(all_data, species)
  
    if(host_names[task_id] %in% host_nats) {
      natrange <- st_read(paste0('./nat_ranges/', host_names[task_id], 
      ...
      if(length(flag_range) > 0) {
        ...
        tryCatch(
          ...
          error=function(err){
            message('On iteration ',task_id, ' there was an error: ',err)
            flag_range2 <<-c(flag_range2,task_id)
          }
        )
  ...
  if(nrow(nat_data) == 0) {
    print("all coordinates are outside of natural range")
  } else {
    ...
    if(nrow(nosea_data) == 0) {
      ...
      if(nrow(nobot_data) == 0) {
        ...
        # for the 20 species in the final_finish_list with over 100,000 coordinates:
        # subsample 10% of remaining coordinates before running spthin
        ...
        species <- rep(host_list[task_id], times = nrow(thinned_data))
        ...
        # writing to file
        write.csv(thinned_data, file = paste0('./occ_data/', host_names[task_id], '.csv'))
      }
    }
  } 
    }
